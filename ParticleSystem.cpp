...
/* Function makes linked list of particles with different behaviors and takes them
 * out of screen if they are out of bounds or explode like firework particles.
 * Links particles and keeps track with head and tail */
...
ParticleSystem::~ParticleSystem() { //destructor
    while (head != nullptr) {
        ParticleCell* next = head->next;
        delete head;
        head = next;
    }
}

int ParticleSystem::numParticles() const { //keep track of number of particles
    return numParticle;
}

void ParticleSystem::add(const Particle& data) { //adds particle to list at the end, changing head nad tail
    if (data.x >= SCENE_WIDTH || data.y >= SCENE_HEIGHT || data.lifetime < 0 || data.x < 0
            || data.y < 0) {
        return;
    }
    ParticleCell* cell = new ParticleCell;
    cell->particle = data;
    cell->next = nullptr;
    cell->prev = nullptr;

    ParticleCell* oldTail = tail;
    if (numParticle == 0) { //case where particle is both head and tail
        head = cell;
        tail = cell;
        numParticle++;
        return;
    ...
    tail = cell; //new one
    oldTail->next = tail;
    tail->prev = oldTail; //previous pointer adjusted
    numParticle++;
    return;
    /*if numparticle is 0, add and make the head and tail the one I just added and make the previous and next equal to null pointer, then make
     * numparticle equal to 1*/
    /*if numparticle greater or equal to 1, add and make tail the new one, don't change head, numparticle++*/
}
...
void ParticleSystem::drawParticles() const {
    ...
    while (curr != nullptr) {
        drawParticle(curr->particle.x, curr->particle.y, curr->particle.color); //given function to draw particles
        curr = curr->next;
    }
    return;
}

ParticleSystem::ParticleCell* ParticleSystem::removeParticle(ParticleCell* curr) { //removes particle helper if out of bounds or lifetime less than 0
    ParticleCell* next = curr->next;
    if (curr == head) {
        ParticleCell* temp = curr->next;
        if (temp != nullptr) {
            temp->prev = nullptr;
        }
        head = temp;
        ...
        ParticleCell* temp = curr->next; //tracker to not lose curr pointer and point at null
        curr->prev->next = temp;
...
void ParticleSystem::moveHelper(ParticleCell* curr) { //move function
    curr->particle.x = curr->particle.x + curr->particle.dx;
    curr->particle.y = curr->particle.y + curr->particle.dy; //all behaviors of move
    curr->particle.lifetime = curr->particle.lifetime - 1;
}
...
void ParticleSystem::moveParticles() {
    ...
    while (curr != nullptr) {
        ...
        if (curr->particle.type == ParticleType::FIREWORK && curr->particle.lifetime < 0) {
            double oldX = curr->particle.x;
            double oldY = curr->particle.y;
            curr = removeParticle(curr);
            for (int i = 0; i < 50; i++) { //creates new particles after firework explodes
                ParticleCell* cell = new ParticleCell;
                cell->particle.x = oldX;
                cell->particle.y = oldY;
                cell->particle.dx = randomInteger(-3, 3);
                cell->particle.dy = randomInteger(-3, 3);
                cell->particle.lifetime = randomInteger(2, 10);
                cell->particle.color = newColor;
                add(cell->particle);
                delete(cell);
            }
        }
        else if (curr->particle.x >= SCENE_WIDTH || curr->particle.y >= SCENE_HEIGHT || curr->particle.lifetime < 0 || curr->particle.x < 0 || curr->particle.y < 0){
            curr = removeParticle(curr); //sets curr to its next using the return value of theh elper function
        }
        else {
            curr = curr->next; //keeps loop going
        }
    }
    return;
}
...
STUDENT_TEST("Milestone 1: Tail member pointer points to right element.") {
    ...
    /* Makes sure tail pointer isn't null, needs to hold
     * the item we added.
     */
    EXPECT_NOT_EQUAL(system.tail, nullptr);

    /* Make sure the particle's x, y, and color are copied over. */
    EXPECT_EQUAL(system.tail->particle.x, particle.x);
    EXPECT_EQUAL(system.tail->particle.y, particle.y);
    EXPECT_EQUAL(system.tail->particle.color, particle.color);

    /* Make sure the list is wired correctly. */
    EXPECT_EQUAL(system.tail->prev, nullptr);
    EXPECT_EQUAL(system.tail->next, nullptr);
}

STUDENT_TEST("Milestone 5: Ballistic particles work.") {
    ParticleSystem system;
